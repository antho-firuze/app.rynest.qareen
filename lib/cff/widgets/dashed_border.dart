...
class DashedBorder extends Border {
  ...
  _drawPath(Canvas canvas, Path source, Rect rect) {
    ...
    while (distance < (topWidth)) {
      ...
      canvas.drawPath(
          metric.extractPath(distance % metric.length, target % metric.length), top.toPaint()..strokeCap = strokeCap);
    ...
    while (distance < (metric.length - bottomWidth - leftHeight)) {
      ...
      canvas.drawPath(
          metric.extractPath(distance % metric.length, target % metric.length), right.toPaint()..strokeCap = strokeCap);
    ...
    var bottomSpacing = (bottomWidth - (partNumber * dashLength)) / (partNumber);
    ...
    while (distance < metric.length - leftHeight) {
      ...
      canvas.drawPath(metric.extractPath(distance % metric.length, target % metric.length),
  ...
  _drawRadiusPath(Canvas canvas, Path source, Rect rect, BorderRadius? borderRadius) {
    ...
    while (distance < leftHeight + topLeftCorner / 2) {
      ...
      if (distance.isNegative && !target.isNegative) {
        canvas.drawPath(
            metric.extractPath(distance % metric.length, metric.length), left.toPaint()..strokeCap = strokeCap);
        canvas.drawPath(metric.extractPath(0, target), left.toPaint()..strokeCap = strokeCap);
      } else {
        canvas.drawPath(metric.extractPath(distance % metric.length, target % metric.length),
            left.toPaint()..strokeCap = strokeCap);
      }
    ...
    var topSpacing = (topWidth + topRightCorner / 2 + topLeftCorner / 2 - (partNumber * dashLength)) / (partNumber);
