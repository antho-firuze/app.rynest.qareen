...
class UDPSocket {
  ...
  static Future<UDPSocket> bindRandom(
    dynamic host, {
    bool reuseAddress = true,
    bool reusePort = false,
    int ttl = 1,
  ...
  static Future<UDPSocket> bindSimple(
    int port, [
    bool reusePort = false,
  ]) async {
  ...
  static Future<UDPSocket?> bindMulticast(
    String ip,
    int port, [
    bool reusePort = false,
  ]) async {
    ...
    try {
      ...
      log('create multicast failure: $e');
      return null;
    }
  }

  static Future<UDPSocket?> bindBroadcast([
    int port = 0,
    bool reusePort = false,
  ]) async {
    RawDatagramSocket socket = await RawDatagramSocket.bind(
    ...
    try {
      ...
      log('create broadcast failure: $e');
      return null;
    }
  }
  ...
  Future<int> send(List<int> buffer, dynamic address, int port) async {
    ...
    if (address is InternetAddress) {
      // log('-------------------->  send: $buffer | ${buffer.length} | ${address.address} | $port');
      addr = address;
    } else if (address is String) {
      // log('-------------------->  send: $buffer | ${buffer.length} | $address | $port');
